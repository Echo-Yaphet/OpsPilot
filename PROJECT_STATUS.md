...
Alertmanager deliveries and reads. Stage 8 adds an external, explicit-approval rollout
...
network partition and physical-streaming PostgreSQL standby promotion rehearsal. Stage 11 moves
...
- A socketless exporter reads only trimmed per-process CPU counters through an authenticated, allowlisted runtime-executor route backed by the target actuator and exposes per-service CPU usage, collection health, last-success time, and strictly validated per-service alert thresholds.
...
- The existing local `GET /api/v1/verification-policy/status` remains unauthenticated and read-only for compatibility. Peer fan-out uses a separate authenticated read-only endpoint. Control API and the rollout controller use distinct asymmetric proof keys to obtain short-lived RS256 credentials from the external workload identity issuer, bound to workload subject, target node, method, path, operation, expiry and one-time `jti`; peer nodes hold only issuer public trust.
...
- A separate one-shot verification-policy rollout controller accepts only a pre-signed candidate plus explicit rollout approval, independently validates its signature/digest/schema/revision, publishes an isolated canary channel first, and advances the stable channel only after every canary reports the exact accepted revision/digest. It then requires a configured quorum and persists fsync-backed phase audit records. Every phase is bound to a rollout plan digest; after process interruption a replacement strictly parses the audit, rejects plan drift/corruption, reconciles canary/stable content, revalidates canary before stable advancement and returns an already committed result idempotently. It exposes no HTTP server or policy-generation interface.
...
- PostgreSQL incident storage, investigation journals and pgvector event memory now have configurable connection, concurrency-acquisition, statement, lock, idle-transaction and application-call deadlines. Synchronous database work runs outside the API event loop, with a default per-process capacity of eight and a 0.25-second acquisition boundary.
- Formal batch `postgres-fast-fail-r4-20260917` sent eight concurrent recommendation-only writes to the database-isolated primary through the unaffected shared API network. All eight returned HTTP 502; the slowest was 0.094 seconds against the 3-second target. The same node's `/health` returned 200 in 0.006 seconds, and canary concurrently returned HTTP 200.
- After reconnecting the database network, the evaluator waited 16 seconds—longer than the 15-second lower-level resolver delay exposed by two rejected diagnostic batches—and all eight failed incident IDs still returned 404. Primary rejoined without recreation; after standby promotion both existing nodes wrote and cross-read successfully, with 4/4 scoped incidents and zero execution/Verification rows.
- The complete backend suite passed 201 tests. Both Control API images were rebuilt; Compose rendering, smoke, runtime-log mTLS/Loki/Tempo, recommendation-only behavior and all nine Prometheus rules passed.
- This is bounded same-host evidence at concurrency eight, not production capacity, cross-host HA, SLA, zero-data-loss, RPO or RTO evidence. Stage 12 remains pending. Full details are in `docs/evaluations/postgres-fast-fail-r4-report.md`.

Latest implementation verification for the Stage 12 external failure-domain acceptance contract:

- `ExternalFaultDomainPlan` rejects same-host/same-zone Control API, PostgreSQL or issuer topologies; the report requires network-partition fail-closed behavior without delayed commit, surviving and replacement node visibility, provider-confirmed database primary change behind a stable endpoint, redundant issuer continuity, trust digest continuity, peer first-use/replay/target denials, exact incident preservation and zero execution/Verification side effects.
- The CLI requires raw `control-node`, `database-provider`, `identity-issuer` and `database-audit` files under one evidence root. It rejects absolute/escaping paths, symlinks, missing files and SHA-256 mismatches before a report can pass. Plan, observation, summary and report artifacts are read-only and same-name batches cannot overwrite them.
- Six focused Stage 12 tests passed, including three distinct failure-domain validators, fail-closed missing provider/identity/trust/safety evidence, raw-file digest verification and immutable output. The rebuilt complete backend suite passed 193 tests.
- Both Control API images were rebuilt and recreated. Final Compose rendering and smoke passed with service/dependency health, runtime-log mTLS, Loki, Tempo and recommendation-only non-execution; Prometheus validated all nine rules.
- This verifies the evaluator and its fail-closed contract only. No real independent-host or managed-HA batch has run, so Stage 12 is not complete and no new HA, SLA, zero-data-loss, RPO or RTO claim is made. The external procedure is documented in `docs/evaluations/stage12-external-fault-domain-protocol.md`.

Latest verification for Stage 11 verification-policy workload identity:

- Formal batch `stage11-policy-identity-r2-20260916` used the controller's dedicated RSA proof key to obtain request-bound peer credentials from the independent issuer, reached exact canary acceptance and then 2/2 quorum at signed revision `2026091602`. Both nodes reported the same digest, strict signature required/valid and final `converged` state.
- The live identity matrix passed: controller proof issuance 200; external peer credential first use 200; issuer proof nonce replay 401; missing peer credential 401; retired HS256 credential 401; peer credential replay 401; wrong-target credential 401; unknown peer target rejected by the issuer with 403.
- The issuer now allowlists the `control-api` and `verification-policy-rollout-controller` workloads for only the policy-peer audience/read operation and known node targets. The controller has a separate proof key pair; callers do not receive issuer signing material and peer nodes mount only issuer public trust.
- Policy bundle HMAC signing remains a distinct content-integrity boundary. Explicit rollout approval, strict candidate validation, canary exact acceptance, stable publication, quorum and fsync audit order were unchanged. The recommendation-only incident created no execution or Verification side effect.
- Focused configuration, issuer, distribution and controller regression passed 53 tests; the complete backend suite passed 187 tests. Raw plan, candidate, audit, result and identity/rollout verification artifacts are read-only under ignored `work/policy-rollouts/stage11-policy-identity-r2-20260916/`.
- The rebuilt default stack passed final smoke with service/dependency health, runtime-log mTLS, Loki, Tempo and recommendation-only behavior. Prometheus validated all nine rules; the execution identity matrix remained 401 missing / 403 unknown target / 200 first use / 401 replay / 404 raw route. Live Redis remained `recommendation_ready`, then `awaiting_approval`, then `resolved / verified=true` only after explicit approval.
- This proves the external issuer seam on one Docker Desktop host. It does not prove cloud-native identity federation, issuer HA, production key custody/rotation or cross-host failure-domain behavior.
- Full evidence and limitations are documented in `docs/evaluations/stage11-policy-identity-r2-report.md`.
...
- Formal batch `stage9-controller-resume-r1-20260916` terminated the controller with exit code 75 immediately after the `canary_accepted` audit record was flushed and fsynced. The checkpoint proved canary revision `2026091601` was accepted while stable remained at baseline revision `2026091600`.
- A replacement process used the same candidate, plan and audit, emitted `rollout_resumed`, revalidated the exact canary revision/digest, published stable once and reached 2/2 quorum. Final rollout reporting returned `converged`; recommendation-only analysis had no execution or Verification side effect.
- All durable entries carried one rollout plan digest. The validation proved `candidate_validated` and `canary_published` occurred once; focused tests also cover post-stable interruption, completed-operation idempotency, plan drift and corrupt-audit rejection.
- The rebuilt current-source suite passed all 182 backend tests. After restoring default mode, a real Redis regression retained all three boundaries: recommendation-only returned `recommendation_ready` without execution, unapproved execution stopped at `awaiting_approval`, and explicit approval restarted only Redis and reached `resolved / verified=true`.
- Final Compose rendering and runtime status were clean; smoke passed with service/dependency health plus runtime-log mTLS/Loki/Tempo checks, and deployed Prometheus validation found all nine rules valid.
- Full evidence and limitations are documented in `docs/evaluations/stage9-controller-resume-r1-report.md`; raw artifacts remain under ignored `work/policy-rollouts/`.

Latest verification for Stage 8 external policy rollout/quorum control:

- Formal batch `stage8-policy-rollout-r2-20260915` validated signed revision `2026091501`, published it to the isolated canary channel, observed exact canary acceptance, then advanced stable and reached 2/2 quorum. The existing rollout report returned `converged` with both nodes online, distribution healthy, no desired conflict and no pending node.
- The accepted digest was `sha256:73f87a67f99d04911e76ae00a367bc93002b1f69e6f9a47f23624e617e85bbf3`; both nodes reported signature required/valid and the primary recommendation-only probe produced no execution or Verification side effect.
- Shared revision history now has accepted signed revision `2026091501`; any future strict signed bundle must use a greater revision, or the identical digest for an idempotent replay of that revision.
- Failed batch `stage8-policy-rollout-r1-20260915` exposed an evaluation-orchestration dependency recreation. The canary gate timed out on the node's strict unknown-key rejection and the controller stopped before stable publication. Volume inspection retained canary revision `2026091501` and stable revision `2026091500`, proving the fail-closed ordering. Adding `--no-deps` to the one-shot validation invocation fixed only the harness lifecycle.
- The rebuilt current-source suite passed all 179 backend tests. Thirteen focused distribution/controller tests cover authenticated channel reads, explicit approval, invalid signature, same-revision conflicts, canary rejection preserving stable, ordered publish, exact quorum, pending minority reporting and audit records.
- After restoring the default local policy mode, real Redis acceptance remained recommendation-only with no execution, missing approval returned `awaiting_approval`, and explicit approval restarted only Redis and reached `resolved`, `verified=true`. Final Compose/smoke validation is recorded below.
- Final default-mode recreation left both Control API nodes and the full stack healthy. Compose rendered cleanly, smoke passed with runtime-log mTLS/Loki/Tempo/dependency checks, and Prometheus validated all nine alert rules.
- Full evidence and limitations are documented in `docs/evaluations/stage8-policy-rollout-r2-report.md`; raw artifacts remain under ignored `work/policy-rollouts/`.
...
- The rebuilt current-source suite passed all 170 backend tests. Focused workflow/persistence coverage passed 47 tests, including ordered Redis then MySQL execution, whole-plan policy denial, partial executor failure, joint target verification and multi-row policy/execution audit persistence. Four additional tests cover Stage 6 reliability aggregation, ordered-boundary failure, invalid-sample accounting and immutable artifacts. The Dashboard production image also built successfully.
- Dedicated batch `stage6-combined-r5-20260915` completed 5/5 valid real Redis+MySQL recovery trials with zero infrastructure-invalid samples. Every trial produced the combined deterministic root cause, Redis then MySQL recommendations, two allowed policy decisions, Redis then MySQL execution, `resolved / verified=true`, and passing independent service/container/dependency probes. PostgreSQL retained all 10 ordered policy rows and five combined execution audits. Mean successful latency was 10.073 seconds; the 5/5 Wilson 95% interval is 56.6%-100%, so this remains a small local-topology result rather than a production SLA.
...
- The default Control API persisted incident snapshots, normalized audit data, investigation journals and the future Skill registry in PostgreSQL. The one-time advisory-lock-protected SQLite import retained its 271 original incidents; after final smoke the live database contained 285 incidents, 14 investigation runs and an empty `skill_versions` registry. pgvector 0.8.6 was healthy with 43 stored event-memory rows, 16 of them unexpired at the final check. Filtered memory was observed in the real Qwen3.5 investigation context without controlling mandatory evidence or actions.
...
- The current-source suite passed 103 backend tests with the existing single LangGraph deprecation warning. Compose, Prometheus configuration, all nine Prometheus rules, real Promtail 3.5.3 syntax, the rebuilt/deployed 21-service stack, and final smoke passed.
- Compose and live inspection proved that no service mounts a Docker socket. The runtime executor has only identity, audit-data, and private actuator-socket volumes on its internal network; Control API is absent from that network. Each actuator is networkless, read-only, `no-new-privileges`, and limited to `CAP_KILL`, while its target joins the actuator-owned PID namespace.
- Runtime identity checks returned 401 without identity, 403 for an externally authenticated unknown target, 200 on first use, 401 on replay, and 404 for a raw route. Independent Gateway and runtime-executor SQLite stores contain successful externally issued identity audit and replay-consumption records.
- A bounded payment CPU fault fired `ContainerHighCPU` at about `1.0006` cores and cleared afterward. Incident `e63a03ee-a5e4-4f3d-b240-2a9079268ec5` remained `alert_resolved`, recommendation-only, confidence `0.9`, with no execution or Verification.
- A real Redis outage produced recommendation-only incident `3cfa2c41-e9f3-4f34-806c-b66a238fe086`, approval-blocked incident `4b0e522d-aba6-4db2-ba2f-912a687750c9`, and approved recovery incident `c4f2aae3-f820-491e-bf77-6015f15ba68c`, `resolved`, `execution_result="restarted redis"`, `verified=true` on verification attempt two. The Redis actuator remained running across workload replacement.
...
- The rebuilt current-source suite passed all 104 backend tests with the existing single LangGraph deprecation warning. Compose validation, real Vault 1.20.3 Agent configuration parsing, Promtail 3.5.3 syntax validation, `promtool` validation with nine rules, the rebuilt/deployed 15-service stack, and final smoke passed.
- A real Vault 1.20.3 dev instance stored the complete, previously accepted CRL-enabled bundle as one KV v2 revision. A real Vault Agent authenticated through an AppRole with a one-use Secret ID and the repository's read-only policy, wrote its token sink with mode `0600`, and atomically rendered KV metadata version plus all bundle data through the checked-in template.
- The host controller consumed that real render, preserved its immutable Vault snapshot and accepted revision, passed the existing certificate/key/hostname/EKU/CN/CRL checks, and executed the strict gateway-trust -> rolling-clients -> gateway-identity sequence. Promtail retained the same container/process, all three services recovered, missing-client/wrong-hostname/authenticated Loki delivery checks passed, and final smoke remained green.
- An isolated user stop produced freshness `0/1/1`, fired only `ServiceLogCollectionStale{service="user-service"}`, and kept the reused incident recommendation-only; recovery cleared all firing alerts. A bounded payment CPU fault fired and resolved `ContainerHighCPU` while incident `e63a03ee-a5e4-4f3d-b240-2a9079268ec5` stayed non-executing with no Verification. A real Redis outage produced recommendation-only incident `d73c6ac0-e052-4b83-996e-41ec72ddb1f7`, approval-blocked incident `6c398fd8-3e9e-4908-a30d-96688341b6f6`, and approved Gateway/Proxy recovery incident `1ddffe3e-2b43-488c-8e17-7cd92a1be7f1`, `resolved`, `verified=true` on attempt three.
- Gateway returned 401 without identity, 403 for an authenticated unknown target, then 401 on credential replay. Proxy returned 401 without identity, 200 for allowlisted stats, 403 for denied stats, and 404 for the raw Docker route. Control API could not resolve the isolated Proxy, and live inspection confirmed only Proxy mounted the Docker socket. Focused coverage also verifies exact render schema, private-key modes, immutable snapshots, revision rollback/conflict rejection, and success-only state advancement.
...
- The rebuilt current-source suite passed all 98 backend tests with the existing single LangGraph deprecation warning. Compose validation, real Promtail 3.5.3 syntax validation, `promtool` validation with nine rules, the rebuilt 15-service stack, and enhanced smoke passed.
- Docker inspection confirmed user/order/payment use `tcp+tls://host.docker.internal:1514`, strict server verification, the local CA, and distinct client certificate/key paths. A live client without a certificate was rejected; a valid client using the wrong server name failed certificate verification; the allowlisted payment client completed mTLS and its unique RFC5424 probe reached Loki.
- The TLS gateway and Promtail run as `nobody`. Promtail mounts only its configuration, server certificate/key, and CA public certificate; it cannot read the CA private key or any client private key and still has no Docker socket, host log, target, or positions mount. The plaintext syslog listener binds only `127.0.0.1:1515` inside the same container.
- Loki retained `compose_service` and `/opspilot-<service>-1` `container` labels for all three services. All freshness series ended at `1`, no syslog parsing-error series existed, and no alert remained firing. Stopping only user made freshness `0/1/1`, fired `ServiceLogCollectionStale`, and reused recommendation-only incident `90cc70f6-700e-44f0-8be2-dd83bd498a93` without execution or Verification; restart restored freshness.
- A bounded payment CPU fault reached about `1.003` cores, fired `ContainerHighCPU`, and persisted recommendation-only incident `e63a03ee-a5e4-4f3d-b240-2a9079268ec5` with confidence `0.9`, no execution, and no Verification. A real Redis outage produced `dependency_up=0`; recommendation-only did not execute, missing approval left Redis exited with `awaiting_approval`, and explicit approval restarted it through Gateway/Proxy to incident `70617a80-ac9f-48a4-957f-2f553cab4c77`, `resolved`, `verified=true` on verification attempt two under the default unsigned policy.
- Gateway returned 401 without identity, 200 once then 401 on replay, and 403 for an authenticated unknown target. Proxy returned 401 without identity, 200 for allowlisted stats, 403 for a denied stats target, and 404 for the raw Docker route. Control API could not resolve the isolated Proxy; only Proxy mounted the Docker socket.
...
- `scripts/runtime-log-vault-agent-controller.py`: strict Vault KV render parser, monotonic revision guard, immutable snapshotter, and success-only bridge into initial install or three-stage rotation.
...
- Shared PostgreSQL has passed a bounded two-process active-active Compose batch plus a same-host database-network partition, eight-request fast-failure/isolation batch and physical-streaming standby promotion rehearsal. Stage 12 now rejects same-host plans and defines the required external evidence, but no real external batch exists yet. PostgreSQL has not been tested across real hosts or availability zones, with automatic leader election/fencing, old-primary rejoin, sustained high-concurrency saturation or a managed HA endpoint. SQLite remains only a local fallback.
...
- Authenticated pull distribution, per-node validation/cache fallback, externally issued request-bound replay-safe peer status, bounded convergence reporting and an external canary-first quorum controller are implemented. The controller now recovers from enumerated process interruptions while its persistent volume survives, but remains bounded deployment coordination rather than distributed consensus; the issuer, distributor and rollout volume remain single-host, and the Stage 7-11 same-host results are not a production HA topology or cloud identity-federation proof.
...
### Completed: authenticated mTLS runtime log transport

- Upgraded all three Docker syslog drivers to `tcp+tls` with strict server-name validation and distinct locally issued client certificates without changing RFC5424 tags or Loki labels.
- Added a same-container TLS gateway that requires CA-verified client certificates and an allowlisted service CN, then forwards only to Promtail on `127.0.0.1:1515`; no new service, socket mount, host-log path, or public API was introduced.
...
### Completed: Vault Agent runtime-log Secret delivery

- Added a concrete Vault KV v2 publisher, minimal read-only policy, AppRole Agent template/configuration, and a host-side successful-render hook without changing the provider-neutral directory contract or adding a Docker socket mount.
...
### Completed: OS/runtime-enforced least-privilege executor

- Removed the Docker SDK, Docker socket mount, and privileged Proxy service from the default stack while preserving all typed Gateway and stats HTTP contracts.
- Added an identity-verifying runtime broker with independent persistent replay/audit storage and private per-target Unix-socket dispatch.
- Added five networkless, read-only target actuators with actuator-owned PID namespaces, `no-new-privileges`, and only `CAP_KILL`; target-specific command matching and persistent quarantine state constrain stop/restart to one workload.
