...
Last updated: 2026-09-15 (Stage 8 external policy rollout/quorum controller)
...
node quorum. The next resume-driven node is broader cross-host/network-partition and PostgreSQL
failover validation before production-quality claims.
...
- Authenticated pull distribution, per-node validation/cache fallback, request-bound replay-safe peer status, bounded convergence reporting and an external canary-first quorum controller are implemented. The controller is bounded deployment coordination rather than distributed consensus; the distributor/rollout volume remain single-host, peer identity still uses a local shared HMAC key, and neither the Stage 7 nor Stage 8 same-host result is a production HA topology.
...
- Completed the external explicit-approval canary-first rollout/quorum controller; next test cross-host node loss, network partitions, controller restart/resume and PostgreSQL failover, then replace local peer HMAC with external workload identity.
...
> Continue OpsPilot from `/Users/yaphet/code/OpsPilot`. Read `AGENTS.md`, `PROJECT_STATUS.md`, `README.md`, `docs/agent-evolution-roadmap.md` and the Stage 5-8 reports, then refresh Git and runtime status. Stages 1-5 implement optional Agents SDK investigation, the approval-gated repair lab, resumable PostgreSQL checkpoints with aggregate budgets and deterministic compaction, filtered pgvector event memory, Tempo Trace, authenticated Skill promotion, and label-isolated paired held-out evaluation. Stage 6 adds deterministic Redis+MySQL multi-target planning, whole-plan fail-closed policy review, ordered execution and joint Verification. Stage 7 validates a two-node shared-store active-active profile. Stage 8 adds an external one-shot policy rollout controller: pre-signed candidate plus explicit approval, canary exact-acceptance gate, atomic stable publication, configured quorum and fsync audit. Formal batch `stage8-policy-rollout-r2-20260915` reached canary 1/1 and quorum 2/2 at revision `2026091501`; failed batch r1 proved stable stayed at its baseline when canary did not accept. These are same-host coordination results, not distributed consensus or production HA. Next validate cross-host/network partitions, controller resume and PostgreSQL failover. Preserve HTTP APIs, `IncidentState`, `IncidentWorkflow.run(request) -> IncidentState`, `OpsTools`, Dashboard evidence, Alertmanager recommendation-only behavior, independent policy/approval gates, socketless target actuators, monotonic Verification revisions, and protected IDE/system files. Kubernetes production rollout remains optional and is not the current resume-project priority.
