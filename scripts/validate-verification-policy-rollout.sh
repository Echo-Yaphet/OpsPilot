...
evaluation_id=${STAGE8_EVALUATION_ID:?STAGE8_EVALUATION_ID is required}
baseline_revision=${STAGE8_BASELINE_REVISION:-2026091500}
candidate_revision=${STAGE8_CANDIDATE_REVISION:-2026091501}
key_id=opspilot-stage8-local-v1
signing_key=opspilot-stage8-local-signing-key
distribution_token=opspilot-stage8-local-distribution-token
peer_key=opspilot-stage8-local-peer-key
...
docker compose --profile policy-rollout run --rm --no-deps policy-rollout-controller \
  ...
  --nodes-json '{"control-api-primary":"http://control-api:8080","control-api-canary":"http://control-api-canary:8080"}' \
