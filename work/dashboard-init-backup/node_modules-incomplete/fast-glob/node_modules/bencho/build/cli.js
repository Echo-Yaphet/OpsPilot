...
var require_table = __commonJS({
  "node_modules/easy-table/table.js"(exports, module) {
    var ANSI_REGEX = require_ansi_regex()();
    var wcwidth;
    try {
    ...
    Table.print = function(obj, format3, cb) {
      var opts = format3 || {};
      format3 = typeof format3 == "function" ? format3 : function(obj2, cell2) {
        for (var key in obj2) {
    ...
    Table.log = function(obj, format3, cb) {
      console.log(Table.print(obj, format3, cb));
    };
...
var CommandsManager = class {
  ...
  build(commands) {
    ...
    return commands.map((command2, index) => {
      return this.#buildCommand(this.#getCommandName(index), command2, {});
    });
  }
  #getParameterCombinations(parameters) {
    return main_default(parameters.map((item) => item.values));
  }
  #getCommandParameters(values) {
    const parameters = {};
  ...
  #buildCommand(name, original, parameters) {
    return {
      name: name === void 0 ? void 0 : this.#formatCommand(name, parameters),
      original,
  ...
  #formatCommand(command2, parameters) {
    return command2.replace(/{(\w+)}/g, (substring, placeholder) => {
      const value = parameters[placeholder];
      if (this.#settings.strictParameters && value === void 0) {
        throw new UnknownParameterPlaceholderError(placeholder);
      }
      return value ?? substring;
    });
  }
  #multipleCommandsByParameters(commands) {
    const result = [];
    const parametersGroups = this.#getParameterCombinations(this.#settings.parameters);
    for (const [index, command2] of commands.entries()) {
      for (const parameterGroup of parametersGroups) {
        ...
        result.push(this.#buildCommand(name, command2, parameters));
      }
    }
    return result;
  }
  #getCommandName(commandIndex) {
...
async function pMap(iterable, mapper, {
  ...
  return new Promise((resolve6, reject_) => {
...
// out/main/executor/command.js
import * as os2 from "node:os";

// out/main/executor/result.js
var AggregateMeasurement = class {
  #measurements = [];
...
var CommandExecutor = class {
  ...
  async execute(command2) {
    await this.#executeWarmup(command2);
    const measurements = await this.#executeRuns(command2);
    const result = new AggregateMeasurement(measurements);
    return { command: command2, result };
  }
  async #executeWarmup(command2) {
    await pTimes(command2.warmupRunsCount, async () => {
      await this.#executor.execute(command2);
    }, { concurrency: 1 });
  }
  async #executeRuns(command2) {
    const concurrency = this.#getRunsConcurrency();
    return pTimes(command2.runsCount, async (index) => {
      const measurement = await this.#executor.execute(command2);
  ...
  #getRunsConcurrency() {
    if (this.#settings.parallel) {
      return Math.max(os2.cpus().length, 1);
    }
    return this.#settings.concurrency;
  }
};

// out/main/reporters/detail.js
var import_easy_table = __toESM(require_table(), 1);
...
// out/main/reporters/reporter.js
var Reporter = class {
  _settings;
...
(function(MarkerType2) {
  MarkerType2["Time"] = "time";
  MarkerType2["Memory"] = "memory";
  MarkerType2["Value"] = "value";
})(MarkerType = MarkerType || (MarkerType = {}));

// out/main/reporters/utils.js
...
// out/main/reporters/detail.js
var DetailReporter = class extends Reporter {
  async report(commands) {
    for (const [index, command2] of commands.entries()) {
      const isLatestCommand = index === commands.length - 1;
      console.log(source_default.cyan(`Benchmark #${index + 1} `));
      this.#reportCommand(command2);
      if (!isLatestCommand) {
        console.log("");
      }
    }
  }
  #reportCommand(command2) {
    const table = new import_easy_table.default();
    const tableLeftPadder = import_easy_table.default.leftPadder(" ");
    const markers = command2.result.combine();
    this.#reportCommandHeader(command2.command);
    ...
    for (const marker of markers) {
      ...
      const formatValue = getValueFormatter(marker.type);
      const formattedMarkerLabel = source_default.yellowBright("Marker");
      const formattedTypeLabel = source_default.blueBright("Type");
      const formattedMeanLabel = source_default.greenBright("Mean");
      const formattedStddevLabel = source_default.cyanBright("StdDev");
      const formattedStddevPercentLabel = source_default.magentaBright("StdDev, %");
      ...
      table.cell(formattedStddevPercentLabel, formatPercentValue(statistic.stddevPercent), tableLeftPadder);
  ...
  #reportCommandHeader(command2) {
    ...
    const lines = [
      ...
      `${source_default.whiteBright(commandLabel)}: ${" ".repeat(labelPaddingLength)}${command2.formatted}`
    ];
    if (command2.formatted !== command2.original) {
      const padding = " ".repeat(commandLabel.length + labelPaddingLength + 1);
      lines.push(`${padding} ${source_default.grey(command2.original)}`);
    }
    const parameters = Object.entries(command2.parameters).map(([key, value]) => `${key}=${value}`).join(", ");
    if (parameters !== "") {
      lines.push(`${source_default.whiteBright(parametersLabel)}: ${parameters}`);
    }
    console.log(`${lines.join("\n")}
`);
  }
  #calculateMarkerStatistic(values) {
    ...
    return {
      ...
      mean: mean2,
      stddev,
      stddevPercent: stddev / mean2 * 100
    };
  }
};

// out/main/reporters/compact.js
var import_easy_table2 = __toESM(require_table(), 1);
var CompactReporter = class extends Reporter {
  async report(commands) {
    const table = new import_easy_table2.default();
    for (const command2 of commands) {
      const markers = this.#calculateCommandStatistic(command2);
      table.cell("Label", command2.command.name ?? command2.command.formatted);
      for (const marker of markers) {
        const formatter = getValueFormatter(marker.type);
        const label = `${marker.label} (${source_default.grey(marker.type)})`;
        const mean2 = formatter(marker.statistic.mean);
        const stderr = formatPercentValue(marker.statistic.stddevPercent);
        table.cell(label, `${source_default.cyan(mean2)} \xB1${stderr}`);
      }
      table.newRow();
    }
    console.log(table.toString());
  }
  #calculateCommandStatistic(command2) {
    const result = [];
    for (const marker of command2.result.combine()) {
      const statistic = this.#calculateMarkerStatistic(marker.values);
      result.push({
        label: marker.label,
        type: marker.type,
...
// out/main/reporters/benchmark.js
var import_easy_table3 = __toESM(require_table(), 1);
var BenchmarkReporter = class extends Reporter {
  async report(commands) {
    const statistic = this.#calculateCommandsStatistic(commands);
    this.#reportMarkers(Object.values(statistic));
  }
  #calculateCommandsStatistic(commands) {
    const result = {};
    for (const command2 of commands) {
      for (const marker of command2.result.combine()) {
  ...
  #reportMarkers(markers) {
    const table = new import_easy_table3.default();
    const tableLeftPadder = import_easy_table3.default.leftPadder(" ");
    for (const marker of markers) {
      if (this._settings.includeMarkers?.includes(marker.label) === false) {
        continue;
      }
      ...
      const formattedStddevLabel = source_default.cyanBright("StdDev");
      const formattedStddevPercentLabel = source_default.magentaBright("StdDev, %");
      const formattedRatioLabel = source_default.redBright("Ratio");
      const formattedLowestLabel = source_default.cyanBright("Lowest");
      const baseline = marker.commands[0];
      const ratios = marker.commands.map((command2) => command2.statistic.mean / baseline.statistic.mean);
      const minRatio = Math.min.apply(null, ratios);
      const maxRatio = Math.max.apply(null, ratios);
      for (const [index, command2] of marker.commands.entries()) {
        ...
        table.cell(formattedMarkerLabel, marker.label);
        table.cell(formattedTypeLabel, marker.type);
        table.cell("Command", command2.command.name ?? command2.command.formatted);
        table.cell(formattedMeanLabel, formatValue(command2.statistic.mean), tableLeftPadder);
        table.cell(formattedStddevLabel, formatValue(command2.statistic.stddev), tableLeftPadder);
        table.cell(formattedStddevPercentLabel, formatPercentValue(command2.statistic.stddevPercent), tableLeftPadder);
        table.cell(formattedRatioLabel, this.#formatRatioValue(ratio, isMinRatio, isMaxRatio), tableLeftPadder);
        table.cell(formattedLowestLabel, isLowest ? source_default.cyanBright("yes") : "", tableLeftPadder);
        table.newRow();
      }
      table.newRow();
    }
  ...
  #formatRatioValue(ratio, isMinRatio, isMaxRatio) {
    const value = ratio.toFixed(2);
    if (isMaxRatio) {
      return source_default.redBright(value);
    }
    if (isMinRatio) {
      return source_default.greenBright(value);
    }
    return value;
  }
};

// out/main/reporters/json.js
var JsonReporter = class extends Reporter {
  async report(commands) {
    const result = commands.map((command2) => {
      return {
        command: command2.command,
        result: command2.result.combine().filter((marker) => {
...
// out/main/executor/iteration.js
import { performance } from "node:perf_hooks";

// out/main/executor/measurement.js
...
// out/main/constants.js
var BENCHO_MARKER_PREFIX = "bencho_marker";
var BENCHO_MARKER_RE = new RegExp(`${BENCHO_MARKER_PREFIX}:(?<data>.+)`, "i");

// out/main/executor/marker.js
var _extractMarker, extractMarker_fn, _getMarkerParts, getMarkerParts_fn, _getMarkerLabel, getMarkerLabel_fn, _getMarkerType, getMarkerType_fn, _isMarkerType, isMarkerType_fn, _getMarkerValue, getMarkerValue_fn, _label_accessor_storage, _type_accessor_storage, _value_accessor_storage, _system_accessor_storage;
var _Marker = class {
  constructor(label, type, value, system = false) {
    __publicField(this, "key");
    __privateAdd(this, _label_accessor_storage, void 0);
    __privateAdd(this, _type_accessor_storage, void 0);
    __privateAdd(this, _value_accessor_storage, void 0);
    __privateAdd(this, _system_accessor_storage, void 0);
    this.label = label;
    this.type = type;
    this.value = value;
    this.system = system;
  ...
  static fromString(input) {
    ...
    return __privateMethod(this, _extractMarker, extractMarker_fn).call(this, match);
  }
  get label() {
  ...
  get type() {
    return __privateGet(this, _type_accessor_storage);
  }
  set type(value) {
    __privateSet(this, _type_accessor_storage, value);
  }
  get value() {
  ...
  get system() {
    return __privateGet(this, _system_accessor_storage);
  }
  set system(value) {
    __privateSet(this, _system_accessor_storage, value);
  }
};
var Marker = _Marker;
_extractMarker = new WeakSet();
extractMarker_fn = function(match) {
  const data = match.groups?.["data"] ?? "";
  const [label, type, value] = __privateMethod(this, _getMarkerParts, getMarkerParts_fn).call(this, data);
  return new _Marker(__privateMethod(this, _getMarkerLabel, getMarkerLabel_fn).call(this, label), __privateMethod(this, _getMarkerType, getMarkerType_fn).call(this, type), __privateMethod(this, _getMarkerValue, getMarkerValue_fn).call(this, value));
};
_getMarkerParts = new WeakSet();
getMarkerParts_fn = function(data) {
  const [label, type, value] = data.split(";");
  if (label === void 0 || type === void 0 || value === void 0) {
    throw new InvalidMarkerDataError(data);
  }
  return [label.trim(), type.trim(), value.trim()];
};
_getMarkerLabel = new WeakSet();
getMarkerLabel_fn = function(input) {
...
_getMarkerType = new WeakSet();
getMarkerType_fn = function(input) {
  const type = input.trim();
  if (!__privateMethod(this, _isMarkerType, isMarkerType_fn).call(this, type)) {
    throw new UnknownMarkerTypeError(type);
  }
  return type;
};
_isMarkerType = new WeakSet();
isMarkerType_fn = function(input) {
  return typeof input === "string" && Object.values(MarkerType).includes(input);
};
_getMarkerValue = new WeakSet();
getMarkerValue_fn = function(input) {
...
_label_accessor_storage = new WeakMap();
_type_accessor_storage = new WeakMap();
_value_accessor_storage = new WeakMap();
_system_accessor_storage = new WeakMap();
__privateAdd(Marker, _extractMarker);
__privateAdd(Marker, _getMarkerParts);
__privateAdd(Marker, _getMarkerLabel);
__privateAdd(Marker, _getMarkerType);
__privateAdd(Marker, _isMarkerType);
__privateAdd(Marker, _getMarkerValue);

// out/main/executor/iteration.js
var IterationExecutor = class {
  #settings;
  #executor;
  constructor(settings, executor) {
    this.#settings = settings;
...
var ProcessExecutor = class {
  ...
  async execute(command2, options = {}) {
    const deferred = pDefer();
    const process_ = this.#spawnProcess(command2.formatted);
    if (options.onChunk !== void 0) {
      process_.stderr.on("data", (chunk) => options.onChunk?.(chunk));
      process_.stdout.on("data", (chunk) => options.onChunk?.(chunk));
    }
  ...
  #spawnProcess(command2) {
    const process_ = cp.spawn(command2, {
      shell: true,
  ...
  #pipeStdioToProcess(process_) {
    process_.stderr.pipe(process3.stdout);
    process_.stdout.pipe(process3.stdout);
  }
};

// out/main/main.js
async function main(input, settings) {
  ...
  await pMap(commands, async (command2) => {
    const executedCommand = await commandExecutor.execute(command2);
    executedCommands.push(executedCommand);
  }, { concurrency: 1 });
  await reporter.report(executedCommands);
}
function getReporter(name, settings) {
  if (name === "detail") {
...
// out/cli/utils.js
function parseParameterList(values = []) {
  return values.map((value) => {
    const parts = value.split("=");
    const label = parts[0] ?? "";
    const values2 = parts[1] ?? "";
    if (label.length === 0 || values2.length === 0) {
...
// out/cli/cli.js
var FLOW_GROUP_LABEL = "Flow options";
var COMMAND_OPTIONS_LABEL = "Commands options";
var PARAMETERS_GROUP_LABEL = "Parameters options";
var OUTPUT_GROUP_LABEL = "Output options";
...
await yargs_default(hideBin(process.argv)).scriptName(packageInfo.name).version(packageInfo.version).showHelpOnFail(false).wrap(null).command("* <commands...>", packageInfo.description, (builder) => {
  return builder.positional("commands", {
    type: "string",
    array: true,
    description: [
  ...
  const settings = new Settings({
    ...
    parameters: parseParameterList(args.parameterList),
